...
vec2 ParallaxMapping(vec2 texCoords, vec3 viewDir)
{
	...
	//计算每一层的Size
	float everyLayerDepth = 1.0 / numLayers;
	//当前层的深度
	...
	while(currentLayerDepth < currentDepthMapValue)
	{
		...
		currentLayerDepth += everyLayerDepth;
	}

	vec2 preTexCoords = currentTexCoords + deltaTexCoords;
	float afterDepth = currentDepthMapValue - currentLayerDepth;
	float beforeDepth = texture(depthMap, preTexCoords).r - currentLayerDepth + everyLayerDepth;
