...
struct HalfEdge {
    HalfEdge *next, *prev;
    HalfEdge *mate; // 相反的halfedge

    VertexIter vertex; // 指向的顶点
    FaceIter   face; // 相邻的face
    EdgeIter   edge; // HalfEdge所属的edge
...
// 边
struct Edge {
    HalfEdge *halfedge[2]; // 如果是boundary edge, halfedge[1] == NULL
    int id;
...
struct Vertex {
    double coord[3];
    double normal[3]; //这个发现是周围的face的法线的加权平均
    int id;
	// 以顶点为入射点的一条halfedges
    HalfEdge *neighborHe;
...
public:
    ...
    bool ConstructMeshDataStructure(char *filename); //从OFF文件中构造Mesh
