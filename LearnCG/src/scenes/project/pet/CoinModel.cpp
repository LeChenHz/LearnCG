...
CoinModel::CoinModel(ReadPetBigFile *readPet, PetMathmaticalModel *petModel)
{
	...
	if (choice == 0) 
	{
		...
		glBufferData(GL_ARRAY_BUFFER, coinCount / 2 * 3 * sizeof(GLfloat), NULL, GL_STREAM_DRAW);
...
void CoinModel::draw(glm::mat4& projection, glm::mat4& view)
{
	glm::mat4 model;
	shader->use();
	if (choice == 0)
	{
		...
		glBufferSubData(GL_ARRAY_BUFFER, 0, pointCount * sizeof(GLfloat) * 3, point_position_data);
